...
mqtt_construct_connect_packet(void)
{
  ...
  uint16_t length = sizeof(protocol_string)
    + 1                                 // connect flags
    + strlen_P(mqtt_con_config->client_id) + 2 // client id
    + 2;                                // keep alive
  if (mqtt_con_config->will_topic)
    length += strlen_P(mqtt_con_config->will_topic)
  ...
  // client id
  mqtt_buffer_write_string_P(mqtt_con_config->client_id);

  // will
  if (mqtt_con_config->will_topic) {
    mqtt_buffer_write_string_P(mqtt_con_config->will_topic);

    // in the mqtt protocol this is supposed to be a string...
    ...
    uint16_t payload_length, bool retain)
{
  ...
  uint16_t length = strlen_P(topic) + 2
    + payload_length;
  if (qos > 0)
    length += 2; // message id
  ...
  if (STATE->stage == MQTT_STATE_CONNECT)
  {
    ...
    if (mqtt_con_config->auto_subscribe_topics)
      for (uint8_t i=0; mqtt_con_config->auto_subscribe_topics[i] != NULL; i++)
        mqtt_construct_subscribe_packet_P(mqtt_con_config->auto_subscribe_topics[i]);
